...
# --------------------------------------------------------------------------- #
# LID 1B — gaspedalgivare (4 fält, foten av pedalen i båda captures)
# --------------------------------------------------------------------------- #
def test_pedal_1b_four_fields():
    v = decode_lid(0x1B, _d("02 86 11 1c 00 00 13 92"))
    assert round(v["pedal_track1"], 3) == 0.646
    assert round(v["pedal_track2"], 3) == 4.380
    assert round(v["pedal_demand"], 2) == 0.0     # foten av → 0 %
    assert round(v["pedal_supply"], 2) == 5.01    # 5V-referens
    assert "pedal_track3" not in v and len(v) == 4  # inget femte fält existerar
