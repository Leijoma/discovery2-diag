"""SLABS layer verified against REAL sniffed reference tool traffic (2026-08-07).

Every request/response below is exact bytes from ``references/captures/
slabs_session_20260807.log``. The tests prove that our layer:
  - **writes** byte-identical commands (clear + actuators), and
  - **reads/decodes** the SLABS responses correctly (fault block, VIN, versions).
"""
import pytest
...
def _frame(data: bytes) -> bytes:
    return encode(data, addressed=False)  # unaddressed session frame <len><data><cs>
...
# ---- decoding matches captured frames exactly --------------------------- #
def test_capture_frames_are_wellformed():
    # Confirms that our unaddressed encoding produces EXACTLY the sniffed bytes.
...
def test_read_faults_logged_and_current():
    ...
    responses = {
        ...
        _frame(b"\x21\x47"): _frame(b"\x61\x47" + bytes(16)),  # current = none
    }
...
# ---- WRITE: commands become byte-identical to the reference tool's ------------- #
...
def test_keepalive_sends_bare_3e_matching_capture():
    # Sniffed keepalive (slabs_session_20260807.log) = '01 3e 3f' → '01 7e 7f',
    # i.e. BARE 3E without a sub-byte. An earlier '3E 01' got no response and tore
    # down the session right after connecting.
    responses = {_frame(b"\x3e"): _frame(b"\x7e")}
    ecu, slabs = _slabs(responses)
    with slabs:
        slabs.tester_present()
    assert ecu.sent[-1] == bytes.fromhex("013e3f")  # '01 3e 3f' — bare 3E
...
def test_establish_confirms_session_with_1a_8a():
    # The reference tool ALWAYS sends 1A 8A as the first request after C1 (the sniff).
    # We mirror it and use the response as an acknowledgement that the session is alive.
    ...
    assert _f(b"\x1a\x8a") in ecu.sent                  # the acknowledgement went out
    assert any("session confirmed" in m for m in msgs)


def test_establish_reports_a_dead_session_but_does_not_raise():
    # Tolerant init only looks for C1 in the burst → can give a false positive
    # "session established" followed by zero reads (car 2026-08-18). If the
    # response to 1A 8A is missing the log should say so — but the establishment must not be torn down.
    ecu, slabs = _slabs_over({})                        # no response to 1A 8A
    msgs = []
    with slabs:
        c1 = slabs.establish(sleep=lambda *_: None, progress=msgs.append)
    assert c1.startswith(b"\xc1\x57\x8f")               # the establishment stands
...
def test_establish_tries_functional_addressing_first():
    ...
    assert functional in ecu.sent          # functional is tried …
    assert physical not in ecu.sent        # … and suffices, physical is not needed
    assert any("[funktionell, F1]" in m for m in msgs)   # shows in the log
...
class _RoutineEcu(FakeKLineEcu):
    """Echoes sent frames and answers 71 22 20 to ALL 31 22 routines (params vary)."""
    def send(self, data):
...
def test_abs_power_bleed_frames_match_the_sniff():
    ...
    # 31 22 04 00 49 c4 + 8 zeros (start), 31 22 04 00 40 00 + 8 (stop) — confirmed 2026-08-07
...
def test_clear_faults_reads_the_delayed_54_ack():
    ...
    with slabs:
        slabs.clear_faults()   # should not raise
