...
def test_airbag_establish_slow_init_then_session():
    # slow init 0x5B → keywords; StartDiagnosticSession 10 81 → 50 81 (adresserat)
    ecu, ab = _airbag({_areq(b"\x10\x81"): _aresp(b"\x50\x81")})
    with ab:
        kw = ab.establish()
    assert kw == (0xE9, 0x8F)                 # KW1/KW2 ur fakens slow-init
    assert ecu.slow_init_addr == AIRBAG_ADDRESS


def test_airbag_read_faults_decodes_004_022():
    # 21 02 → 61 02 90 04 90 16 … (belagt ur sniffen), adresserad framing
    resp = {
    ...
    # verifiera att vi FAKTISKT skickade adresserade ramar (82 5b f7 …)
    assert any(bytes(s).startswith(b"\x82\x5b\xf7") for s in ecu.sent)


def test_airbag_is_read_only():
    # Skydds-invariant: airbag-lagret exponerar ingen clear/output/security.
    for attr in ("clear_faults", "output_test", "start_routine", "unlock", "buzzer"):
        assert not hasattr(Airbag, attr), f"Airbag ska INTE ha {attr} (pyro-säkerhet)"
