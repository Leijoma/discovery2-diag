...
def test_lidstore_switches_to_slabs():
    ...
    st.ingest_line("[ 200] 02 21 54 79 04 61 54 07 08 aa")  # 21 54 → heights
    snap = st.snapshot()
...
def test_slabs_any_door_decoded():
    ...
    st.ingest_line("[ 2] 00 06 61 56 01 0f 0f 0f ed")   # byte0 bit0 = 1 → open
    door = next(s for l in st.snapshot()["lids"] if l["lid"] == "56" for s in l["decode"] if s["name"] == "any_door")
    assert door["value"] == "open"
    st.ingest_line("[ 3] 00 06 61 56 00 0f 0f 0f ec")   # byte0 bit0 = 0 → closed
    door = next(s for l in st.snapshot()["lids"] if l["lid"] == "56" for s in l["decode"] if s["name"] == "any_door")
...
def test_menu_items_carry_lid_bindings():
    ...
    door = next(i for g in MENUS["slabs"] if "Switches" in g["cat"] for i in g["items"] if i["name"].startswith("Any Door"))
    assert door["lid"] == "56" and door["sig"] == "any_door"


def test_solve_linear_battery_points():
    # raw 0x35ab=13739 → 13.739 V ; raw 0x2d94=11668 → 11.67 V  (scale ~1/1000)
...
def test_solve_linear_needs_two_distinct():
    ...
    assert solve_linear([(5, 1.0), (5, 2.0)]) is None  # same raw → no slope


def test_automap_numeric_finds_battery_field():
    # two plaintext readings (engine off / running) + raw block for LID 10
    r = automap_solve(
...
def test_automap_numeric_finds_temperature_with_bias():
    # coolant temp from 21 1A@0: (u16/10 − 273.2). 59.2 °C vs 58.2 °C
    r = automap_solve(
...
def test_automap_state_finds_door_bit():
    r = automap_solve(
        [{"text": "open", "raws": {"56": "01 0f 0f 0f"}},
         {"text": "closed", "raws": {"56": "00 0f 0f 0f"}}],
        ["56"],
    )
    assert r["ok"] and r["mode"] == "state"
    assert r["lid"] == "56" and r["offset"] == 0 and r["bit"] == 0
    assert r["mapping"] == {"open": 1, "closed": 0}


def test_automap_diff_shows_changed_byte():
    # settings block read in two modes; only byte 3 changes → that's where the field lives
    r = automap_solve(
    ...
    assert r["offset"] == 3 and r["bit"] == 0  # unambiguous bit → found


def test_automap_diff_empty_when_nothing_changed():
    # same raw bytes in both readings → nothing moved (wrong LID or no change)
    r = automap_solve(
        [{"text": "on", "raws": {"46": "aa bb"}}, {"text": "off", "raws": {"46": "aa bb"}}],
        ["46"],
    )
